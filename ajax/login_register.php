...
function send_mail($uemail, $token, $type)
{
    ...
    $email->setFrom("gyalpo2001@gmail.com", "Pema");
    ...
    if (mysqli_num_rows($u_exist) == 0) {
        ...
        if ($u_fetch['is_verified'] == 0) {
            ...
            if (!send_mail($data['email'], $token, 'account_recovery')) {
                ...
                $date = date("y-m-d");
                $query = mysqli_query($con, "UPDATE `user_cred` SET `token`='$token',`t_expired`='$date' 
                WHERE `id`='$u_fetch[id]' ");
...
if (isset($_POST['recover_user'])) {
    ...
    $query = "UPDATE `user_cred` SET `password`=?, `token`=?,`t_expired`=?
