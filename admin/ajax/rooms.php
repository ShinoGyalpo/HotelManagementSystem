...
if (isset($_POST['add_room'])) {
    ...
    if ($stmt = mysqli_prepare($con, $q3)) {
        ...
        $flag = 0;
        die('query cannot be prepared - room_features');
    }
...
if (isset($_POST['add_image'])) {
    $frm_data = filteration($_POST);
    $img_r = uploadImages($_FILES['image'], ROOMS_FOLDER);
    if ($img_r == 'inv_img') {
        echo $img_r;
...
if (isset($_POST['get_room_images'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        if ($row['thumb'] == 1) {
            $thumb_btn = "<i class='bi bi-check-lg text-light bg-success px-2 py-1 roundedfs-5'></i>";
        } else {
